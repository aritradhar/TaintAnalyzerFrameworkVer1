		...
		{			
			...
			for(int i=0;i<dp.method_count();i++)
			{
				...
				executor.execute(worker);
                        ...
                        if(qry_c>1)
                        {
                            for(int i1=0;i1<dp.method_count();i1++)
                            {
				Runnable worker=new Datalog_exec_TP(s_cp[i1],qry1,current_file_path.substring(0, current_file_path.indexOf(".dl")).concat("_output.txt"));
				executor.execute(worker);
                            }
                        }
		 }
