...
public class caller_analyzer_MT_DIR 
{
        ...
        public static void setDir(String indir,String outdir)
        {
            input_dir=indir;
            output_dir=outdir;
        }
	public static void main(String[] args) throws Exception
	{
		...
		*/
		...
		for(int k=0;k<filename_arr.length;k++)
		{
			...
			dg.main(null);
                ...
                //type check for knowledgebase
                ToStringScanner ts=new ToStringScanner();
                ts.setInputDir(input_dir);
                ts.setOutputDir(output_dir);
                ts.setFile();
                ts.scanner();
                //end 
		...
		*/
                ...
                Thread [][]Tx=new Thread[dl_file_name_arr.length][];
                Thread [][]Tx1=new Thread[dl_file_name_arr.length][];
		for(int k=0;k<dl_file_name_arr.length;k++)
		{			
			...
			*/
                        Tx[k]=new Thread[dp.method_count()];
                        Tx1[k]=new Thread[dp.method_count()];
                        for(int i=0;i<dp.method_count();i++)
				Tx[k][i]=new Thread(new Datalog_exec(s_cp[i],qry,current_file_path.substring(0, current_file_path.indexOf(".dl")).concat("_output.txt")));
			for(int i=0;i<dp.method_count();i++)
				Tx[k][i].start();
                        ...
                        {
                            for(int i=0;i<dp.method_count();i++)
				Tx1[k][i]=new Thread(new Datalog_exec(s_cp[i],qry1,current_file_path.substring(0, current_file_path.indexOf(".dl")).concat("_output.txt")));
                            for(int i=0;i<dp.method_count();i++)
				Tx1[k][i].start();
                        }
                        /*
			...
			for(int i=0;i<dp.method_count();i++)
                ...
                for(int k=0;k<dl_file_name_arr.length;k++)
                {
                    for(int k1=0;k1<Tx[k].length;k1++)
                    {
                        Tx[k][k1].join();
		...
		System.out.println("---- Total Program Execution time : "+totalTime+"ms. ----");
                ...
                OutXML lr=new OutXML(dl_file_name_arr,input_dir,output_dir,qry);
                lr.setTime(totalTime.toString());
